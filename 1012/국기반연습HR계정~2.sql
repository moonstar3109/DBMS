...
select to_char(sysdate, 'YYYY-MM-DD') from dual; --날짜 fmt 형식은 대문자로 표기
select to_char(sysdate, 'YYYY-MM-DD HH:MI:SS') from dual;
select to_char(sysdate, 'YY-MM-DD-HH24:MI') from dual;
select to_char(sysdate, 'YY"년" MM"월" DD "일"')from dual; --날짜 fmt 형식이 아니면 사용이 불가능한 경우 " " 묶어서 표현


select first_name, to_char( hire_date, 'YYYY-MM-DD' )from employees;

--숫자를 문자로 TO_CHAR(숫자, 형식)
select to_char(20000,'999999999') from dual; --9는 자리수의 표현
...
--문자를 숫자로 TO_NUMBER(문자, 혁식)
select '2000'+ 2000  from dual; --자동형변환
select to_number('2222') + 2000 from dual;   --명시적형변환
select '$2000'+ 2000 from dual; -- 문자형이 $ 같은 특정 문자를 지니고 있을 때는 자동형변환이 불가능합니다.
select to_number('$2000' , '$9999') + 2000 from dual;
